...
物理空间$X$上的复平方可积函数构成量子Hilbert空间
$H = L^2(X) = \{ \psi: X \to \mathbb C \mid \int_X |\psi(x)|^2 \text d x < \infty \}$，
$\mathcal L(H) = \text{End} \ H$表示$H$上的全体线性算子，
$\mathcal B(H)$表示$H$上的全体有界线性算子，
它是含幺C*-代数．
自伴算子的全体分别记为$\mathcal L(H)^\dagger$和$\mathcal B(H)^\dagger$，
构成实线性空间．
...
复Hilbert空间$H = (H,\langle \cdot, \cdot \rangle)$
上的有界线性算子集合构成Banach代数$\mathcal B(H)$．
根据 Riesz 表示定理，
...
为了讨论一个复代数元$a \in A$在何种条件下相当于乘法，
即有复数$\lambda$使得$\lambda 1_A = a$在某种条件下成立，
需要讨论

\begin{equation}
\lambda 1_A - a
\end{equation}
\label{eq:diff-lambda-multiplicative}
...
其补集$\rho(a) = \mathbb C \setminus \sigma(a)$称为预解集．
若$A$是Banach代数则$\sigma(a)$是非空紧集．

矩阵代数$A$是典型的非交换代数，
谱集$\sigma(a)$就是矩阵$a$的特征值集合．
$A^\times = \{ a \in A \mid \det(a) \neq 0 \}$．
这里的行列式定义了$a$的特征多项式，
在代数封闭域$\mathbb C$中可分解为：
...
\begin{aligned}
\sigma(a) 
&= \{ \lambda \in \mathbb C \mid \lambda 1_A - a \notin A^\times \} \\
&= \{ \lambda \in \mathbb C \mid p_a(\lambda) = \det(\lambda 1_A - a) = 0 \}
\end{aligned}
$$
...
算子$V\in \mathcal L(H)^\dagger$若满足：
$P^2 = P$称为正交投影算子，
简称投影算子，
非正交的投影算子不是自伴算子，
不在讨论范围．
...
$$
A = \int_X \lambda \text d P_\lambda
$$
...
称为\textbf{乘性线性泛函(multiplicative linear functional)}
或者\textbf{特征(character)}．
对于含幺Banach代数$A$，
线性泛函$\varphi$，
令$\varphi(1_A)=1$，
作为泛函，
$\varphi$作用于$\forall a\in A$
得到$\lambda_a = \varphi(a) \in\mathbb C$．

\begin{framed}
\noindent

考虑$a$在何种条件下相当于$\lambda_a$的乘法，
考虑$\varphi$对(\ref{eq:diff-lambda-multiplicative})的作用：

$$
\varphi(\lambda_a 1_A - a)
= \lambda_a\varphi(1_A) - \varphi(a)
= 0
$$
...
$$
\varphi\big( (\lambda_a 1_A - a)(\lambda_a 1_A - a)^{-1} \big)
= \varphi(\lambda_a 1_A - a)\varphi\big((\lambda_a 1_A - a)^{-1}\big)
= 0
$$
...
故矛盾．
不可逆的$\lambda_a 1_A - a \in A^\times$使得：
...
有\textbf{Gleason-Kahane-Zelazko定理}给出以下等价条件：
\begin{itemize}
  \item $\varphi$非零且是乘性的
  \item 
  $\varphi(1) = 1$，
  且$\forall a \in A^\times:\ \varphi(a) \neq 0$
  \item
  $\forall a \in A:\ \varphi(a) \in \sigma(a)$
\end{itemize}

乘性线性泛函的值在相应元素的谱中．
反之，若线性泛函 $\varphi$ 满足 $\varphi(\mathbf{1})=1$ 且 $\varphi(a) \in \sigma(a)$ 对所有 $a$ 成立，
则可推出 $\varphi$ 是乘性泛函．

含幺Banach代数$A$上的乘性线性泛函$\varphi$是连续的，
且$\| \varphi \| = 1$．
对于$\forall a \in A$，
乘法线性泛函总能将其映射为谱点，
将抽象的代数元转为可以处理的复数：
...
$$
\mathfrak m_\varphi = \text{Ker} \ \varphi \in \text{Max} \ A
$$

\section{\texorpdfstring{$C(X)$}{C(X)}的极大理想}
...
\section{\texorpdfstring{$C(X)$}{C(X)}的拓扑}
...
物理空间$X$上的复平方可积函数构成量子Hilbert空间$H = L^2(X)$，
可观测量对应$\mathcal B(H)^\dagger$中的自伴算子．
以含幺交换von Neumann子代数$V \subseteq \mathcal B(H)$为对象，
$\forall A,B \in V:\ [A,B] = 0$，
以嵌入为态射，
构成偏序范畴$(\mathcal V(H),\leq)$．
这里的对象$V\in\mathcal V(H)$是一组可以同时被赋予确定值的相容可观测量构成的context．
在范畴上构造预层范畴$\textbf{Set}^{\mathcal V(H)^\text{op}}$．

含幺交换von Neumann子代数$V$上，
讨论其含幺乘性线性泛函的集合，
即Gelfand谱$\Delta(V)$，
它具有拓扑结构，
是紧Hausdorff空间．
$P\succeq Q$表示$P$投影到更大的子空间$Q$，
这是一种嵌入偏序，
产生偏序范畴$(\mathcal V(H),\succeq)$．
这一反变函子过程视为预层
$\underline\Sigma \in \textbf{Set}^{\mathcal V(H)^\text{op}}$，
称为谱预层．
产生Gelfand谱中的限制映射：
...
\begin{aligned}
  ...
  V &\mapsto \underline\Sigma(V) = \Delta(V)
\end{aligned}
\end{equation}
\label{eq:spectral-presheaf}

投影算子$P\in P(H)$不一定属于某个由含幺交换von Neumann子代数$V\in\mathcal V(H)$．
需要用$V$中最接近$P$的投影算子来近似$P$．
正交模格范畴
$(P(H),\preceq,1_H,0_H) \simeq (\text{Sub} \ H,\subseteq,H,\{0\})$．
不是分配格，
从而不是Bool代数．
作者引用了Daseinisation的概念，
源自海德格尔的 Dasein（此在/存在），
后缀 "-isation" 表示“使……成为存在/带入存在”的过程．
外Daseinisation从上方逼近，
找$V$中大于等于$P$的最小投影；
内Daseinisation从下方逼近，
找$V$中小于等于$P$的最大投影．

一个投影算子$P\in P(H)$代表一个命题，
而$P$不一定属于某个由含幺交换von Neumann子代数代表的具体context $V\in\mathcal V(H)$．
我们关注从context $V$的视角看$P$，
则需要用$V$中最接近$P$的投影算子来近似$P$．

含幺交换von Neumann子代数$V$中所有的投影算子是集合$\mathcal O(V) = P(V)$，
按照格的结构，
分别构造内dasein和外dasein
预层$\mathcal O,\mathcal I \in \textbf{Set}^{\mathcal V(H)^\text{op}}$，


\section{Kochen‑Specker定理}

Kochen-Specker 定理\cite{KochenSpecker1967}
说明谱预层 $\underline{\Sigma} \in \textbf{PSh}$ 
(\ref{eq:spectral-presheaf})
无全局元素：

$$
\Gamma \Sigma = \varnothing
$$

其中 $\Gamma: \textbf{PSh} \to \textbf{Set}$ 为全局截面函子．


设 $\gamma \in \Gamma\underline{\Sigma}$，定义赋值 $v: B(H)_{\text{sa}} \to \mathbb{R}$ 如下：

\begin{enumerate}
    \item 对任意自伴算子 $\hat{A}$，选取包含 $\hat{A}$ 的交换子代数 $V$（如 $\hat{A}$ 生成的 $C^*$-代数）
    \item 定义 $v(\hat{A}) := \gamma_V(\hat{A})$
\end{enumerate}

\textbf{良定义性}：若 $\hat{A} \in V_1 \cap V_2$，取 $V_3 \supseteq V_1 \cup V_2$，则
\[
\gamma_{V_1}(\hat{A}) = \gamma_{V_3}|_{V_1}(\hat{A}) = \gamma_{V_3}(\hat{A}) = \gamma_{V_3}|_{V_2}(\hat{A}) = \gamma_{V_2}(\hat{A})
\]

\textbf{验证 Kochen-Specker 条件}：
\begin{itemize}
    \item \textbf{值规则}：$\gamma_V(\hat{A}) \in \sigma(\hat{A})$（Gel'fand 谱的性质）
    \item \textbf{函数条件}：若 $\hat{B} = f(\hat{A})$，取 $V$ 同时包含 $\hat{A},\hat{B}$，则
    \[
...
\subsection{\texorpdfstring{Kochen-Specker 赋值 $\Rightarrow$ 全局截面}{Kochen-Specker 赋值 => 全局截面}}
...
\textbf{验证 $\gamma_V$ 是乘法泛函}：
\begin{itemize}
    \item 线性：由 FUNC 可导出 $v$ 在交换子代数上线性
    \item 乘法：$\gamma_V(\hat{A}\hat{B}) = v(\hat{A}\hat{B}) = v(\hat{A})v(\hat{B}) = \gamma_V(\hat{A})\gamma_V(\hat{B})$
    \item 单位元：$\gamma_V(\hat{1}) = v(\hat{1}) = 1$
\end{itemize}

\textbf{一致性}：对 $V' \subseteq V$，$\hat{A} \in V'$，
\[
...
\[
\boxed{\text{存在 Kochen-Specker 赋值} \;\Longleftrightarrow\; \Gamma\underline{\Sigma} \neq \varnothing}
\]

取其逆否命题，Kochen-Specker 定理（$\dim H \ge 3$ 时不存在赋值）等价于：

\[
\boxed{\Gamma\underline{\Sigma} = \varnothing \quad \text{当 } \dim H \ge 3}
\]

\textbf{物理意义}：谱预层 $\underline{\Sigma}$ 无全局截面是 Kochen-Specker 定理在拓扑斯框架中的几何表述．它揭示了量子理论无法具有经典意义的“微观态”（全局赋值），而只能在每个语境 $V$ 中局部地赋值．这正是 D\"oring-Isham 框架引入\textbf{具身化}（daseinisation）和\textbf{拟态}（pseudo-state）来替代传统微观态的根本原因．

\section{Mitchell–Bénabou 语言}

预层函子$\textbf{Set}^{\mathcal V(H)^\text{op}}$是一个topos，
在Mitchell–Bénabou 语言中，
每个预层$F \in \textbf{Set}^{\mathcal V(H)^\text{op}}$
对应一个类型，
类型$\Gamma$对应谱预层$\underline{\Sigma}$，
类型$O$对应外预层$\mathcal O$，
类型$R$对应量值预层$\underline R^\leftrightarrow$，
类型$\Omega$对应子对象分类器．

Kochen-Specker定理说明谱预层$\Sigma$
没有全局元素，
不存在$\gamma \in \Gamma \underline{\Sigma}$，
即不存在$\gamma: \textbf{1} \to \Sigma$．
但M-B语言允许广义元素$x: U \to X$．
...
$$
\braket{\psi}{\phi} = 
\langle \psi,\phi \rangle = \int_X \overline{\psi(x)} \phi(x) \text dx
$$

其中被积项目是复数的Hermite内积，
令$z = x + y \text i, w = u + v \text i$：
...
\subsection{\texorpdfstring{类型理论：形式语言 $L(S)$}{类型理论：形式语言 L(S)}}
...
\subsection{\texorpdfstring{量子理论：在 $\mathbf{Sets}^{\mathcal{V}(H)^{\mathrm{op}}}$ 中的实现}{量子理论：在 Sets\string^\{V(H)\string^\{op\}\} 中的实现}}

\subsubsection{语境范畴 $\mathcal{V}(H)$}
\begin{itemize}
...
\subsubsection{具身化（Daseinisation）——类型到对象的映射}
...
\chapter{附录：范畴论解读Hall2013：Quantum Theory for Mathematicians}

\cite{Hall2013}

$\textbf{Hilb}$为复Hilbert空间范畴，
$\textbf{C}^\star$为C*-代数范畴，
$\textbf{vN}$为冯·诺依曼代数范畴．
$\text{Meas}(X)$为测度空间$X$上的测度范畴．

有限维$H \in \textbf{FdHilb}$上的自伴算子，
在单位正交基下有$n \times n$矩阵表示
$A \in \mathbb C_n^n = \mathcal B(H)^\dagger$，
满足共轭对称$A = A^\dagger$，
相当于$\forall \phi,\psi \in H:\ \langle \phi,A\psi\rangle = \langle A\phi,\psi\rangle$．
$M_n(\mathbb{C}) \in \textbf{C}^\star$作为C*-代数，
矩阵$A$是代数元，
其谱$\sigma(A) \subset \mathbb R$．

无限维情况，
令空间为$X = \mathbb R$，
$H = L^2(X) \in \textbf{Hilb}$，
定义位置算子$Q$使得
...
满足$\forall \phi,\psi \in H:\ \langle \phi,Q\psi\rangle = \langle Q\phi,\psi\rangle$．
它的定义域是：

$$
D(Q) = \left\{ \psi \in H = L^2(X) :\ \int_X |(Q \psi)(x)|^2 \text dx < \infty \right\}
$$

$D(Q) \subset H$且$\overline{D(Q)} = H$，
$Q$是无界稠定算子．
考虑$\lambda \in \mathbb C$和非零的$\psi \in D(Q)$满足：

$$
(Q - \lambda I )(\psi) = x\psi(x) - \lambda \psi(x) = (x - \lambda)\psi(x) = 0
$$

这要求$\psi(x) = 0$对于几乎所有$x \neq \lambda$成立．
因此$\psi$必须是$\lambda$处的狄拉克δ函数，
...
复Hilbert空间$H \in \textbf{Hilb}$上，
它把Borel σ-代数$B(\sigma(A))$中的每个可测集$E$映射为$H$上的一个正交投影算子$\mu(E)$：

$$
\begin{aligned}
\mu: B(\sigma(A)) &\to \text{Proj} \ H \\
E &\mapsto \mu(E)  
\end{aligned}
$$
...
若它是正交模格同态，
则构成了投影值测度。
若视这些集合$B(\sigma(A))$，$\text{Sub} \ H$，$\text{Proj} \ H$为范畴，
则$\mu$是一个函子，
它在有限维中有特征值分解:
