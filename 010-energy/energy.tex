...
能量交换的本质来源于不对称性，而非仅仅来源于力的做功或粒子的运动本身。在标准布朗马达理论中，不对称性通常由空间势能的周期性破缺提供~\cite{Reimann2002Brownian,Hanggi2009Brownian}；然而，本文框架揭示了一种更为根本的不对称性来源——空间自身的度量结构。乘性域 $\mathbb R^+$ 的指数映射将加性对数空间中的对称随机扰动 $\pm\varepsilon$ 转化为物理空间中乘性的非对称位移增量：$|\mathrm e^{+\varepsilon}-1| \neq |\mathrm e^{-\varepsilon}-1|$。正是这一度量层面的不对称性，构成了持续能量交换的根源。即使粒子的位置完全自由、不受任何势阱或周期约束，只要力场 $F(x)$ 依赖于位置 $x$，功的微元 $\omega = F(x)\wedge\mathrm dx$ 的期望 $\mathbb E[\omega] = \frac12 F(x)x\sigma^2\,\mathrm dt$ 便严格为正，表明系统能够持续从热环境中提取净功。该不对称性驱动的能量交换机制在多个物理系统中获得独立印证。在对数势 $U(x)\propto\ln|x|$ 中的Brown运动研究揭示了几何势能本身足以打破遍历性，使系统的长时间行为偏离平衡态预期~\cite{Dechant2011Logarithmic, Dechant2012Logarithmic}——这正是对称性破缺在动力学层面的表现。在随机动力学层面，乘性噪声驱动的系统已被证明能产生持续的定向概率流~\cite{Volpe2014Brownian, Lindner2001Brownian}，其物理根源恰恰在于噪声强度对状态的依赖将对称的随机激励转化为非对称的响应。在非平衡热力学层面，布朗回转器通过各向异性温度场（温度张量的非对称性）驱动粒子产生定向旋转并对外输出功~\cite{Filippetti2021Brownian, Sasa2014Stochastic}，明确展示了几何非对称性本身就是功输出的源泉。最优输运理论~\cite{Villani2009Optimal}与信息几何~\cite{Amari2016}则为这类不对称性驱动的能量转换提供了定量框架：能量提取的上界由状态空间度量与参考测度之间的$L^2$-Wasserstein距离所决定，最大功率对应于几何曲率的最优配置。综上，不对称性是能量交换的必要条件；乘性域结构所提供的度量不对称性，使得粒子即便在完全自由扩散的条件下也能持续输出净功，这一机制构成了本文理论框架的核心物理直觉。
