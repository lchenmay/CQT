...
波动率是对数价格的标准差。
用移动平均的时间窗更新，
带有衰减的移动平均可以描述波动率跳空收敛的过程。
GARCH模型通过极大似然估计确定方差方程的参数，
描述波动率的历史变化并有一定预测能力。
...
Garman-Klass波动率等。
我们采用Yang-Zhang波动率，
波动率分解为隔夜跳空、日内波动、趋势分量三部分
\cite{YangZhang2000Volatility}。

设第 $t$ 期的 OHLC 价格为 $O_t, H_t, L_t, C_t$，样本窗口长度为 $n$。定义如下三个分量。

隔夜波动率：
\begin{equation}
\sigma_o^2 = \frac{1}{n-1} \sum_{t=1}^{n} \left( \ln \frac{O_t}{C_{t-1}} - \bar{\mu}_o \right)^2, \qquad
\bar{\mu}_o = \frac{1}{n} \sum_{t=1}^{n} \ln \frac{O_t}{C_{t-1}}
\end{equation}

开收盘波动率：
\begin{equation}
\sigma_c^2 = \frac{1}{n-1} \sum_{t=1}^{n} \left( \ln \frac{C_t}{O_t} - \bar{\mu}_c \right)^2, \qquad
\bar{\mu}_c = \frac{1}{n} \sum_{t=1}^{n} \ln \frac{C_t}{O_t}
\end{equation}

Rogers-Satchell 日内波动率：
\begin{equation}
...
到$w$个bar时间窗口的归一化波动率：
\begin{equation}
\overline \sigma = \sigma \sqrt{\frac{w}{n}}
\end{equation}
...
下面这几篇文献涵盖了从理论框架到具体金融应用的多个层面。

Choi \cite{Choi2025LevyGeometry} 于2025年发表的论文直接构建了 Lévy 过程的信息几何结构。该工作推导了 Lévy 过程的 $\alpha$-散度，并识别了其上的 Fisher 信息矩阵和 $\alpha$-联络。该框架被具体应用于研究金融建模中常用的 CGMY 模型、方差 Gamma 模型和 Merton 模型，为理解带有跳跃和厚尾特征的金融模型提供了深刻的几何视角。

Lin \cite{Lin2026FinancialRelativity} 于2026年提出了一个更为宏大的理论框架——“金融相对论”。该框架将资产定价中的物理测度 $P$ 和风险中性测度 $Q$ 解释为不同信息约束下的几何结构，而资产价格则被解释为终期收益在信息子空间上的几何投影。这一框架试图统一地解释价格波动、事件驱动行为和风险溢价。

Marti 等 \cite{Marti2021ReviewNetworks} 于2021年发表了一篇跨度很大的综述，回顾了二十年来对金融市场相关性、层级结构和网络聚类的研究。该综述整合了来自信息几何、经济物理学、计量经济学等多个领域的观点，是理解如何用信息几何方法分析金融市场网络结构的重要参考资料。

Wolfer 和 Watanabe \cite{Wolfer2023MarkovGeometry} 于2023年发表的综述详细阐述了马尔可夫核（转移矩阵）的信息几何。该工作构建了不可约随机矩阵集合上的 Fisher 度量和对偶仿射联络，并探讨了时间可逆性、集总性（lumpability）所带来的几何结构，对金融中常见的状态转移建模（如信用评级迁移、宏观状态切换）具有直接的理论价值。

Turc \cite{Turc2024PortfolioGeometry} 在 BNP Paribas 的前沿量化金融研讨会上提供了一份业界视角的应用资料，展示了如何利用信息几何中的散度函数来改进投资组合优化。通过将散度函数应用于风险预算框架，可以统一多种经典方法，并增强组合的稳健性。
